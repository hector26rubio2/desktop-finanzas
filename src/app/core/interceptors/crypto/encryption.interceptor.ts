...
function tryDecrypt(event: HttpEvent<unknown>, logger: LoggerService): Observable<HttpEvent<unknown>> {
  if (event instanceof HttpResponse) {
    ...
    if (b && typeof b === 'object' && b['encrypted']) {
      return from(
        ...
        catchError((err) => {
          logger.error('Decryption failed; passing response through unmodified.', err);
          return of(event);
        }),
      );
    }
...
export const encryptionInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (body && typeof body === 'object' && !skipEncrypt) {
    return from(encrypt(body)).pipe(
      ...
      switchMap((event: HttpEvent<unknown>) => tryDecrypt(event, logger)),
    );
  }

  return next(req).pipe(switchMap((event: HttpEvent<unknown>) => tryDecrypt(event, logger)));
};
