...
describe('financialFlowContribution', () => {
  ...
  it('counts only loan interest as expense', () => {
    expect(
      financialFlowContribution(movement({ operationType: 'LoanPayment', interestComponent: 12, trmApplied: 4 })),
    ).toEqual({ income: 0, expense: 48 });
  });

  it('keeps ordinary income, purchases and credit interest in operating flow', () => {
    expect(financialFlowContribution(movement({ type: 'Income', amountBase: 250 }))).toEqual({
      income: 250,
      expense: 0,
    });
    expect(financialFlowContribution(movement({ operationType: 'CreditPurchase', amountBase: 80 }))).toEqual({
      income: 0,
      expense: 80,
    });
    expect(financialFlowContribution(movement({ operationType: 'CreditInterest', amountBase: 5 }))).toEqual({
      income: 0,
      expense: 5,
    });
  });
