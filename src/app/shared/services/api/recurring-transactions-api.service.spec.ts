...
class RecurringMemoryLocal {
  ...
  seed<T extends { id: string }>(kind: LocalKind, value: T) {
  ...
  async list<T>(kind: LocalKind): Promise<T[]> {
    return [...(this.docs.get(kind)?.values() ?? [])] as T[];
  }
  async get<T>(kind: LocalKind, id: string): Promise<T | null> {
    return (this.docs.get(kind)?.get(id) as T) ?? null;
  }
...
describe('RecurringTransactionsApiService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [RecurringTransactionsApiService, { provide: LocalDataRepository, useValue: local }],
    });
  ...
  it('materializes every due occurrence exactly once and advances the template atomically', async () => {
    ...
    expect(movements.map((item) => item.id)).toEqual([
      ...
      'rent:occurrence:2026-08-02',
      'rent:occurrence:2026-08-03',
    ]);
